...
namespace SFC.Models
{
    public class OrderService
    {
        ...
        public static async System.Threading.Tasks.Task Process(int orderId, Dictionary<string, int> items, String request)
        {
            Dictionary<int, FoodTask> classification = new Dictionary<int, FoodTask>();

            foreach (var item in items)
            {
                int vendorId = menu.getFood(Int32.Parse(item.Key)).vendorIDService;
                if (!classification.ContainsKey(vendorId))
                {
                    FoodTask task = new FoodTask(orderId, request);
                    task.addTask(Int32.Parse(item.Key), item.Value);
                    classification.Add(vendorId, task);
                }
                else
                {
                    classification[vendorId].addTask(Int32.Parse(item.Key), item.Value);
                }

                // Update Menu
                await menu.changeQuantity(Int32.Parse(item.Key), menu.getFood(Int32.Parse(item.Key)).quantity - item.Value);
            }
